...
export const DoctorDashboard = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 overflow-hidden">
      {/* Header */}
      <header className="bg-gradient-doctor backdrop-blur-xl border-b border-blue-200/50 px-6 sm:px-8 py-5 flex justify-between items-center shadow-colored-blue relative overflow-hidden z-10">
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 via-transparent to-indigo-600/10"></div>
        <div className="flex items-center gap-4 relative z-10">
          <div className="bg-white/20 backdrop-blur-md p-3 rounded-xl shadow-glow border border-white/30">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-full lg:w-1/3 bg-white/80 backdrop-blur-xl border-r border-blue-100/50 flex flex-col shadow-deep-lg z-0">
          <div className="p-6 sm:p-8 border-b border-blue-100/50 bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-white/80 backdrop-blur-sm">
            <div className="flex items-center gap-4 mb-2">
              <div className="bg-gradient-doctor p-3 rounded-xl shadow-colored-blue flex-shrink-0">
                <Brain className="w-6 h-6 text-white" />
              </div>
              <div className="min-w-0">
                ...
                <p className="text-xs text-blue-600/80 font-medium mt-1">Análisis en tiempo real del historial completo</p>
              </div>
            </div>
          </div>
          
          <div className="flex-1 overflow-y-auto p-6 sm:p-8 bg-gradient-to-b from-blue-50/30 via-white/50 to-indigo-50/20">
            {!patient && (
              <div className="text-center text-gray-400 mt-20 animate-fade-in px-4">
                <div className="bg-gradient-to-br from-blue-100 to-indigo-100 w-20 h-20 mx-auto mb-5 rounded-2xl flex items-center justify-center shadow-soft">
                  <Search className="w-10 h-10 text-blue-400" />
                </div>
                <p className="font-medium text-gray-500 mb-1">Busca un paciente para ver el análisis</p>
                <p className="text-xs text-gray-400">Ingresa la cédula del paciente en la barra superior</p>
              </div>
            )}
            ...
            {analyzing && (
               <div className="flex flex-col items-center justify-center mt-10 space-y-5 animate-fade-in px-4">
                 <div className="bg-gradient-doctor p-5 rounded-2xl shadow-colored-blue">
            ...
            {!analyzing && aiOpinion && (
               <div className="markdown-content text-sm text-gray-700 animate-fade-in">
                 <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-blue-100/50 shadow-deep card-modern">
        ...
        <div className="w-full lg:w-2/3 flex flex-col bg-gradient-to-br from-white/60 via-blue-50/30 to-indigo-50/20">
           {patient ? (
             <>
               {/* Patient Header */}
               <div className="bg-white/90 backdrop-blur-xl border-b border-blue-100/50 p-6 sm:p-8 shadow-deep">
                 <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                   <div className="flex items-center gap-5 flex-1 min-w-0">
                     <div className="h-20 w-20 bg-gradient-doctor rounded-2xl flex items-center justify-center text-white text-2xl font-black shadow-colored-blue border-4 border-white flex-shrink-0">
                        {patient.full_name.substring(0,2).toUpperCase()}
                     </div>
                     <div className="min-w-0 flex-1">
                       <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-3">{patient.full_name}</h2>
                       <div className="flex flex-wrap gap-3 text-sm text-gray-600">
                         <span className="flex items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-lg font-semibold">
                           <User className="w-4 h-4 text-blue-600 flex-shrink-0" /> {patient.cedula}
                         </span>
               ...
               <div className="flex-1 overflow-y-auto p-6 sm:p-8">
                 <div className="flex items-center gap-3 mb-8">
                   <div className="h-1 w-12 bg-gradient-doctor rounded-full flex-shrink-0"></div>
                 ...
                 <div className="space-y-6">
                   {records.map((record, index) => (
                     <div key={record.id} className="relative pl-10 sm:pl-12 animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                       ...
                       <div className="bg-white/90 backdrop-blur-sm p-6 sm:p-7 rounded-2xl border border-blue-100/50 shadow-deep hover:shadow-deep-lg hover:scale-[1.01] transition-all duration-300 card-modern ml-4">
                         <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-5">
                           <div className="flex-1 min-w-0">
                             ...
                             <h4 className="font-black text-xl text-gray-900 mb-2">{record.diagnosis || 'Sin Diagnóstico'}</h4>
                           </div>
                           <span className="text-xs text-gray-500 font-bold bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200 flex-shrink-0">
                             {new Date(record.record_date).toLocaleDateString()}
                           </span>
                         </div>
                         <p className="text-gray-700 text-sm mb-5 leading-relaxed font-medium">{record.description}</p>
                         
                         {(record.treatment || record.notes) && (
                           <div className="bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-blue-50/80 p-5 rounded-xl border border-blue-100/50 space-y-4 shadow-soft">
                             {record.treatment && (
                               <div className="flex flex-col sm:flex-row gap-3">
                                 <span className="font-bold text-blue-700 text-xs uppercase tracking-wider min-w-[100px] flex-shrink-0">Tratamiento:</span>
                                 <span className="text-gray-700 text-sm font-medium">{record.treatment}</span>
                               </div>
                             )}
                             {record.notes && (
                               <div className="flex flex-col sm:flex-row gap-3">
                                 <span className="font-bold text-indigo-700 text-xs uppercase tracking-wider min-w-[100px] flex-shrink-0">Notas:</span>
                   ...
                   {records.length === 0 && (
                     <div className="text-center py-16 text-gray-400 animate-fade-in px-4">
                       <div className="bg-gradient-to-br from-blue-100 to-indigo-100 w-24 h-24 mx-auto mb-5 rounded-2xl flex items-center justify-center shadow-soft">
                         <FileText className="w-12 h-12 text-blue-400" />
                       </div>
                       <p className="font-semibold text-gray-500 mb-1">No hay registros médicos disponibles</p>
                       <p className="text-xs text-gray-400">El historial clínico aparecerá aquí cuando haya registros</p>
                     </div>
                   )}
                 </div>
               </div>
             ...
             <div className="h-full flex flex-col items-center justify-center text-gray-400 animate-fade-in px-4">
               ...
               <p className="font-bold text-gray-500 text-lg mb-2">Selecciona un paciente</p>
