...
export const AdminDashboard = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-slate-50 via-emerald-50/30 to-teal-50/20">
      {/* Header */}
      <header className="bg-gradient-admin backdrop-blur-xl border-b border-emerald-200/50 px-6 py-4 flex justify-between items-center shadow-colored-emerald relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-600/10 via-transparent to-teal-600/10"></div>
        <div className="flex items-center gap-3 relative z-10">
          <div className="bg-white/20 backdrop-blur-md p-2.5 rounded-xl shadow-glow-emerald border border-white/30">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-100/80 font-medium">Análisis Inteligente de Datos</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden max-w-7xl mx-auto w-full p-6 gap-6">
        ...
        <div className="flex-1 flex flex-col bg-white/80 backdrop-blur-xl rounded-3xl shadow-deep-lg border border-white/50 overflow-hidden">
          ...
          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gradient-to-b from-slate-50/50 via-white/30 to-emerald-50/20" ref={scrollRef}>
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                <div className={`max-w-[80%] ${msg.role === 'user' 
                  ...
                  rounded-2xl p-5 transition-all duration-300 card-modern`}>
                  <div className={`flex items-center gap-2.5 mb-3 ${msg.role === 'user' ? 'border-b border-white/20' : 'border-b border-emerald-100'} pb-2.5`}>
                    <div className={`p-1.5 rounded-lg ${msg.role === 'user' ? 'bg-white/20' : 'bg-emerald-100'}`}>
                  ...
                  {/* SQL Code Block */}
                  {msg.sql && (
                    <div className="mt-4 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl p-4 font-mono text-xs text-emerald-400 overflow-x-auto border border-emerald-500/20 shadow-glow-emerald">
                      <div className="flex items-center gap-2 text-emerald-300/70 mb-2 text-[10px] uppercase tracking-wider font-semibold">
                  ...
                  {msg.data && (
                    <div className="mt-4 bg-gradient-to-br from-emerald-50 via-teal-50/50 to-emerald-50 rounded-xl p-5 border border-emerald-200/50 shadow-soft">
                      {msg.data.type === 'single' && (
                         <div className="flex items-baseline gap-2">
                           <div className="text-4xl font-black text-emerald-700 bg-white/60 px-4 py-2 rounded-xl shadow-sm">{msg.data.value}</div>
                            ...
                            <thead>
                              <tr className="bg-gradient-to-r from-emerald-100 to-teal-100">
                                {msg.data.columns.map((col: string) => (
                                  <th key={col} className="px-4 py-3 text-xs font-bold text-emerald-900 uppercase tracking-wider border-b-2 border-emerald-200">
                                    {col}
                                  </th>
                                ))}
                              </tr>
            ...
            {loading && (
              <div className="flex justify-start animate-fade-in">
                <div className="bg-white/90 backdrop-blur-sm border border-emerald-200/50 rounded-2xl p-4 shadow-deep flex items-center gap-3 text-emerald-700 text-sm font-medium">
          ...
          {/* Input Area */}
          <div className="p-6 bg-gradient-to-r from-white/80 via-emerald-50/30 to-white/80 backdrop-blur-xl border-t border-emerald-100/50">
            <form onSubmit={handleSend} className="relative flex items-center gap-3">
              ...
              <button 
                ...
                className="p-4 bg-gradient-admin hover:from-emerald-600 hover:to-emerald-700 disabled:from-slate-300 disabled:to-slate-400 text-white rounded-2xl transition-all shadow-colored-emerald hover:shadow-glow-emerald-lg hover:scale-105 disabled:scale-100 disabled:shadow-none flex items-center justify-center"
              >
                <Send className="w-5 h-5" />
              </button>
            </form>
            <p className="text-center text-xs text-emerald-600/70 mt-3 font-medium">
